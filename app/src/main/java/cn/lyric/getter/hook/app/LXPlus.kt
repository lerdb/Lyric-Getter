...
object LXPlus : BaseHook() {

    override fun init() {
        super.init()
        val lyricModuleClass = loadClassOrNull("top.lxmusic.plus.mobile.lyric.LyricModule")
        lyricModuleClass.isNotNull { clazz ->
            ...
            loadClass("android.view.WindowManagerImpl").methodFinder().first { name == "addView" }.createHook {
                after { view ->
                    if (view.args[0]::class.java.name.contains("top.lxmusic.plus.mobile.lyric")) {
